...
def download_patches(socket):
    ...
    # PERSISTENT Hook 3/4: Overwrite tLC_TX and tLC_RX function pointers directly in the
    # firmware binary so they point to our _installed_fptr_3/_4 shims.
    # This makes hooks 3/4 persistent — no runtime RAM write needed after flash.
    # The PoC code starts at: 0x8010A000 + final_offset_len (with ISA bit +1)
    poc_base = 0x8010A000 + final_offset_len
    # _installed_fptr_3 is at PoC offset 0x216, _installed_fptr_4 at 0x2B2
    fptr3_ram_addr = poc_base + 0x216 + 1  # +1 for MIPS16e ISA bit
    fptr4_ram_addr = poc_base + 0x2B2 + 1
    print(f"Persistent Hook 3 (tLC_TX): _installed_fptr_3 = 0x{fptr3_ram_addr:08x}")
    print(f"Persistent Hook 4 (tLC_RX): _installed_fptr_4 = 0x{fptr4_ram_addr:08x}")

    # Patch tLC_TX pointer at patch-data offset 0x3AA0 (file offset 0x7220, RAM 0x8010DAA0)
    lc_tx_patch_offset = 0x7220 - g_patch_start  # 0x7220 - 0x3780 = 0x3AA0
    old_lc_tx = int.from_bytes(final_full_data[lc_tx_patch_offset:lc_tx_patch_offset+4], 'little')
    final_full_data[lc_tx_patch_offset:lc_tx_patch_offset+4] = fptr3_ram_addr.to_bytes(4, 'little')
    print(f"  tLC_TX @ offset 0x{lc_tx_patch_offset:04x}: 0x{old_lc_tx:08x} -> 0x{fptr3_ram_addr:08x}")

    # Save original tLC_TX pointer to backup_addr_3 location in our PoC data
    # backup_addr_3 stores at RAM 0x80133FF4 — we write this at runtime during _start_install_fptrs
    # But we also need to ensure the backup gets the ORIGINAL value, not our hook.
    # Since _start_install_fptrs reads from the pointer location (which now has our hook),
    # we need to write the original value to backup_addr_3 in RAM separately.
    # Solution: write the original value into the backup_addr_3 DATA WORD in our PoC code.
    # Find backup_addr_3 in the PoC code — it stores 0x80133FF4 (the RAM address).
    # We can't easily change it to store the original value directly in the PoC...
    # Instead: the Patch Writer will write the backup values after download.

    # Patch tLC_RX pointer at patch-data offset 0x3A68 (file offset 0x71E8, RAM 0x8010DA68)
    lc_rx_patch_offset = 0x71E8 - g_patch_start  # 0x71E8 - 0x3780 = 0x3A68
    old_lc_rx = int.from_bytes(final_full_data[lc_rx_patch_offset:lc_rx_patch_offset+4], 'little')
    final_full_data[lc_rx_patch_offset:lc_rx_patch_offset+4] = fptr4_ram_addr.to_bytes(4, 'little')
    print(f"  tLC_RX @ offset 0x{lc_rx_patch_offset:04x}: 0x{old_lc_rx:08x} -> 0x{fptr4_ram_addr:08x}")
...
g_poc_buf = bytearray([,
...
def main():
    ...
    #########################################################################
    # Write backup pointers for Hooks 3/4
    # The hook pointers are now persistent in the firmware binary (patched above).
    # But the backup addresses (where shims jump to call the original handler)
    # are in uninitialized RAM — we must write the original handler addresses.
    #########################################################################

    print("[*] Writing backup pointers for persistent Hooks 3/4...")
    # Hook 3 backup: original tLC_TX handler = 0x80042421
    backup_addr_3 = 0x80133FF4
    write_and_confirm_with_read(socket, backup_addr_3, 0x80042421)
    print(f"  backup_addr_3 (0x{backup_addr_3:08X}) = 0x80042421 (original tLC_TX)")

    # Verify Hook 3 is persistent in firmware
    data = read(socket, 0x8010DAA0)
    if data is not None:
        val = int.from_bytes(data, 'little')
        if val != 0x80042421:
            print(f"[+] Hook 3 PERSISTENT: tLC_TX = 0x{val:08x} (our shim)")
        else:
            print(f"[-] Hook 3 NOT persistent: tLC_TX still = 0x{val:08x} (original)")


    # Hook 4 backup: original tLC_RX handler = 0x80042189
    backup_addr_4 = 0x80133FEC
    write_and_confirm_with_read(socket, backup_addr_4, 0x80042189)
    print(f"  backup_addr_4 (0x{backup_addr_4:08X}) = 0x80042189 (original tLC_RX)")

    # Verify Hook 4 is persistent in firmware
    data = read(socket, 0x8010DA68)
    if data is not None:
        val = int.from_bytes(data, 'little')
        if val != 0x80042189:
            print(f"[+] Hook 4 PERSISTENT: tLC_RX = 0x{val:08x} (our shim)")
        else:
            print(f"[-] Hook 4 NOT persistent: tLC_RX still = 0x{val:08x} (original)")
